	...
	internal sealed class DataBranchRerouteElement : VisualElement
	{
		...
		sealed class ConnectManipulator : DragManipulator
		{
			...
			void UpdateBezier(Vector2 mousePosition)
			{
				...
				if (hoverSlot != null)
				{
					...
					else
					{
						...
						{
							_Bezier.endPosition = new Vector2(hoverNode.position.x, hoverSlot.position.center.y);
						}
						else
						{
			...
			void UpdateMousePosition(Vector2 graphMousePosition)
			{
				...
				DataSlotGUI.UpdateHoverSlot(node.nodeGraph, node, null, slot, graphMousePosition);
			...
			protected override void OnMouseUp(MouseUpEvent e)
			{
				...
				else if (hoverSlot != null)
				{
					...
					branch = graphEditor.ConnectDataBranch(hoverNode.nodeID, hoverObj, hoverSlot, node.nodeID, null, slot);
					if (branch != null)
					{
