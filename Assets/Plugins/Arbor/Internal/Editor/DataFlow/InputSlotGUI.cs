			...
			switch (current.GetTypeForControl(controlID))
			{
				...
				case EventType.MouseDrag:
					if (GUIUtility.hotControl == controlID && current.button == 0)
					{
						...
						UpdateHoverSlot(nodeGraph, node, targetObject, inputSlot, graphView.GUIToGraph(current.mousePosition));
						...
						if (current.button == 0)
						{
							...
							if (graphEditor != null)
							{
								...
								else if (_HoverSlot != null)
								{
									...
									branch = graphEditor.ConnectDataBranch(node.nodeID, targetObject, inputSlot, _HoverNode.nodeID, _HoverObj, _HoverSlot);
									if (branch != null)
									{
