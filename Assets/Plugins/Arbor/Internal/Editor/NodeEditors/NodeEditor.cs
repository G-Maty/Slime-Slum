	...
	public abstract class NodeEditor : ScriptableObject
	{
		...
		void InitializeNodeElement()
		{
			...
			if (graphEditor != null)
			{
				...
				if (IsWindow())
				{
					graphView.nodeLayer.Add(_NodeElement);
				}
				else
				{
					graphView.groupNodeLayer.Add(_NodeElement);
				}
			}
		...
		void InitializeMinimapNodeElement()
		{
			...
			_MinimapNodeElement = new MinimapNodeElement(this);
		...
		void OnChangeNodeCommentViewMode()
		{
			if (isShowableComment && NodeGraphEditor.IsShowNodeComment(node))
			{
				...
				{
					graphEditor.hostWindow.nodeCommentLayer.Add(_CommentElement);
				}
			}
			else
			{
		...
		{
			return node.position.Contains(position);
		}
