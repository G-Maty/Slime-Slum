...
namespace ArborEditor
{
	...
	public sealed class StateBehaviourEditorGUI : BehaviourEditorGUI
	{
		...
		void SetNodeGraph(NodeGraph nodeGraph)
		{
			...
			{
				if (!ReferenceEquals(_NodeGraph, null))
				{
					_NodeGraph.onPlayStateChanged -= OnPlayStateChanged;
					_NodeGraph.stateChangedCallback -= OnStateChanged;
				}
		...
		void SetStateBehaviour(StateBehaviour stateBehaviour)
		{
			...
			{
				if (!ReferenceEquals(_StateBehaviour, null))
				{
		...
		internal void UpdateStateLinkTargetPosition(int targetNodeID)
		{
			for (int stateLinkIndex = 0; stateLinkIndex < _StateLinkElements.Count; stateLinkIndex++)
			{
				...
				{
					stateLinkElement.DoChangedPosition();
				}
			}
		}
		...
		internal bool DeleteKeepConnection(int deleteNodeID, int nextStateID, Vector2 nextEndPosition, Vector2 nextEndControl)
		{
			bool changed = false;
			...
			for (int stateLinkIndex = 0; stateLinkIndex < _StateLinkElements.Count; stateLinkIndex++)
			{
				...
				if (stateLink.stateID == deleteNodeID)
				{
					...
					stateLinkElement.bezier.endPosition = nextEndPosition;
					stateLinkElement.bezier.endControl = nextEndControl;
