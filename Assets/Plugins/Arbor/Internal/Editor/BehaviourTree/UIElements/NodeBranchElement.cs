...
namespace ArborEditor.BehaviourTree.UIElements
{
	...
	internal sealed class NodeBranchElement : VisualElement
	{
		...
		void OnContextClick(ContextClickEvent evt)
		{
			...
			menu.AddItem(EditorGUITools.GetTextContent(Localization.GetWord("Go to Parent Node") + " : " + _GraphEditor.GetNodeTitle(parentNode)), false, () =>
			{
				_GraphEditor.BeginFrameSelected(parentNode);
			});
			menu.AddItem(EditorGUITools.GetTextContent(Localization.GetWord("Go to Child Node") + " : " + _GraphEditor.GetNodeTitle(childNode)), false, () =>
			{
