...
namespace ArborEditor.UIElements
{
	...
	internal sealed class InputSlotElement : VisualElement
	{
		...
		void OnChangedPosition()
		{
			...
			{
				pinPos.x = _BehaviourEditorGUI.nodeEditor.node.position.x;
			}
		...
		sealed class ConnectManipulator : DragManipulator
		{
			...
			void UpdateBezier(Vector2 graphMousePosition)
			{
				...
				var hoverNode = DataSlotGUI._HoverNode;
				...
				if (hoverSlot != null)
				{
					...
					{
						targetPos = new Vector2(hoverNode.position.xMax, hoverSlot.position.center.y);
			...
			void UpdateMousePosition(Vector2 graphMousePosition)
			{
				...
				DataSlotGUI.UpdateHoverSlot(slotGUI.nodeGraph, slotGUI.node, slotGUI.targetObject, slotGUI.slot, graphMousePosition);
			...
			protected override void OnMouseUp(MouseUpEvent e)
			{
				...
				var slotGUI = slotElement.slotGUI;
				var nodeGraph = slotGUI.nodeGraph;
				var node = slotGUI.node;
				...
				if (hoverSlot != null)
				{
					...
					branch = graphEditor.ConnectDataBranch(node.nodeID, targetObject, inputSlot, hoverNode.nodeID, hoverObj, hoverSlot);
					if (branch != null)
					{
