...
namespace ArborEditor
{
	...
	public class NodeGraphEditor : ScriptableObject
	{
		...
		internal bool _RebuildingNodeEditors = false;
		...
		bool RebuildNodeEditor(bool onInitialize)
		{
			bool rebuilt = false;
			bool removed = false;
			...
			if (nodeGraph != null)
			{
				...
				for (int nodeIndex = 0; nodeIndex < nodeCount; nodeIndex++)
				{
					...
					{
						nodeEditor.Validate(node, onInitialize);
					}
				}
			}
		...
		{
			bool rebuilt = RebuildNodeEditor(false);
		...
		private static int s_PopupButtonControlHash = "s_PopupButtonControlHash".GetHashCode();
		...
		public VisualElement ShowPopupButtonControl(Vector2 attachPoint, GUIContent content, int activeControlID, GUIStyle style, System.Action<Rect> onClick)
		{
			if (_PopupButton == null)
			{
				...
				_HostWindow.popupLayer.Add(_PopupButton);
			}
		...
		void EndRename(bool acceptChanges)
		{
			var renameOverlay = GetRenameOverlay();
		...
		CalculatorNode CreateCalculatorInternal(Vector2 position, System.Type calculatorType)
		{
			...
			if (calculator != null)
			{
				...
				BeginRename(calculator.nodeID, calculator.name, true);
			}
		...
		void CreateComment(Vector2 position)
		{
			...
			if (comment != null)
			{
				...
				BeginRename(comment.nodeID, comment.name, true);
			}
		...
		{
			var parameterContainer = _NodeGraph?.parameterContainer;
