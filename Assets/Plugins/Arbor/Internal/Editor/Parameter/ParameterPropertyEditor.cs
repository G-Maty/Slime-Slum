...
namespace ArborEditor
{
	...
	internal sealed class ParameterPropertyEditor : PropertyEditor
	{
		...
		static class Defaults
		{
			...
			static Defaults()
			{
				valueContent = EditorGUITools.GetTextContent("Value");
				notFoundValueContent = EditorGUITools.GetTextContent("Error: not found value");
				getContent = EditorGUITools.GetTextContent("Get");
				setContent = EditorGUITools.GetTextContent("Set");
		...
		void DoGUI()
		{
			...
			_LayoutArea.TextField(EditorGUITools.GetTextContent(label), _NameProperty, LayoutArea.Width(_LayoutArea.rect.width - 20f));
