	...
	internal sealed class GraphSettingsWindow : PopupWindowContent
	{
		...
		void DoGUI()
		{
...
#endif
			{
				...
				{
					_HostWindow.OnChangedShowLogo(true);
				}
			}

			float zoomLevel = _HostWindow.graphView.graphScale.x * 100;
			EditorGUI.BeginChangeCheck();
			zoomLevel = _LayoutArea.Slider(EditorContents.zoom, zoomLevel, ArborEditorWindow.k_ZoomMin, ArborEditorWindow.k_ZoomMax) / 100f;
			if (EditorGUI.EndChangeCheck())
			{
				Vector2 center = _HostWindow.graphView.isLayoutSetup ? _HostWindow.graphView.graphViewport.center : Vector2.zero;
				_HostWindow.graphView.SetZoom(center, new Vector3(zoomLevel, zoomLevel, 1f), true);
			}
			...
			if (EditorGUI.EndChangeCheck())
			{
				_HostWindow.OnChangedNodeCommentAffectsZoom();
			}
