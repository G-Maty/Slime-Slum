...
namespace ArborEditor.Events
{
	...
	internal sealed class PersistentCallEditor : PropertyEditor
	{
		...
		private static class Default
		{
			...
			static Default()
			{
				targetContent = EditorGUITools.GetTextContent("<Target>");
			}
		}
		...
		void DoGUI()
		{
			using (new ProfilerScope("PersistentCallEditor.DoGUI"))
			{
				...
				for (int argumentIndex = 0; argumentIndex < argumentProperties.Count; argumentIndex++)
				{
					...
					GUIContent nameLabel = EditorGUITools.GetTextContent(name);
				...
				if (returnType != null)
				{
					...
					GUIContent returnLabel = EditorGUITools.GetTextContent("Return");
				...
				if (targetType != null && targetType.IsValueType)
				{
					...
					GUIContent outputInstanceLabel = EditorGUITools.GetTextContent("<Output Instance>");
